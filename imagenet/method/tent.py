...
NORM_MODULE_TYPES = (nn.BatchNorm2d, nn.GroupNorm, nn.LayerNorm)
...
def collect_params(model):
    ...
    Walk the model's modules and collect all normalization affine parameters.
...
def check_model(model):
    ...
    assert not has_all_params, "tent should not update all params: " \
                               "check which require grad"
    has_norm = any([isinstance(m, NORM_MODULE_TYPES) for m in model.modules()])
