...
class EATA(nn.Module):
    ...
    def __init__(
        ...
        self.original_eata = original_eata
    ...
    def forward(self, x):
        ...
        if self.steps > 0:
            for _ in range(self.steps):
                outputs, num_counts_2, num_counts_1, updated_probs = forward_and_adapt_eata(
                    x,
                    self.model_arch,
                    self.model0,
                    self.model,
                    ...
                    original_eata=self.original_eata,
                )
...
def forward_and_adapt_eata(
    ...
    if criterion != "ent":
        ...
        if resnet_or_vit == 'Standard_R50':
            model1.model.fc = nn.Identity() 
            cos_sim = F.cosine_similarity(model1(x).unsqueeze(1), model.model.fc.weight, dim=2) 
        elif resnet_or_vit == 'Standard_VITB':
            model1.head = nn.Identity() 
            cos_sim = F.cosine_similarity(model1(x).unsqueeze(1), model.head.weight, dim=2) 
        else:
    ...
    if criterion != "ent":
        ...
        num_counts_2 = entropys_ind.size(0)
    ...
    if fishers is not None:
        ...
        loss += ewc_loss
    if not original_eata:
        loss -= alpha[0] * softmax_mean_entropy(outputs)

    should_update = not original_eata or x[ids1][ids2].size(0) != 0
    if should_update:
        loss.backward()
        optimizer.step()
    optimizer.zero_grad()
    return outputs, num_counts_2, filter_ids_1[0].size(0), updated_probs
