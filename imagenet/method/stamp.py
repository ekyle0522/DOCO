...
class STAMP(nn.Module):
    def __init__(self, model, optimizer, alpha, num_class):
        ...
        self.alpha = alpha
        logger.info(f"===== self.alpha for STAMP is {self.alpha}🚧 ===== ")
        self.margin = alpha * math.log(num_class)
        logger.info(f"===== self.margin = alpha * math.log(num_class) for STAMP is {self.margin}🚧 ===== ")
    ...
    def update_memory(self, x):
        x_views = list(x) if isinstance(x, (list, tuple)) else [x]
        x_origin = x_views[0]
